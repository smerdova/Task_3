...
class MainPage(BasePage):
    ...
    def click_first_element(self):
        self.find_element(MainPageLocators.first_element).click()

    @allure.step('Проверяем, что надпись "Соберите бургер" отображается')
    def check_assemble_the_burger_title(self):
        element = self.find_element(MainPageLocators.assemble_the_burger_title)
        
        assert element.is_displayed()

    @allure.step('Ищем исходный элемент и перетаскиваем его') 
    def drag_and_drop_first_element(self):
        self.wait(3).until(expected_conditions.visibility_of_element_located(MainPageLocators.assemble_the_burger_title))
        source = self.find_element(MainPageLocators.first_element)
        target = self.find_element(MainPageLocators.target_element)
        self.drag_and_drop(source, target)

    @allure.step('Ищем третий элемент и перетаскиваем его') 
    def drag_and_drop_third_element(self):
        self.wait(3).until(expected_conditions.visibility_of_element_located(MainPageLocators.assemble_the_burger_title))
        source = self.find_element(MainPageLocators.third_element)
        target = self.find_element(MainPageLocators.target_element)
        self.drag_and_drop(source, target)
    ...
    def drag_and_drop_seventh_element(self):
        self.wait(3).until(expected_conditions.visibility_of_element_located(MainPageLocators.assemble_the_burger_title))
        source = self.find_element(MainPageLocators.seventh_element)
        target = self.find_element(MainPageLocators.target_element)
        self.drag_and_drop(source, target)

    @allure.step('Проверяем, что увеличивается каунтер данного ингредиента')
    def check_counter_is_incremented(self):
        element = self.find_element(MainPageLocators.counter)
    ...
    def click_place_an_order_button(self):
        self.find_element(MainPageLocators.place_an_order_button).click()
